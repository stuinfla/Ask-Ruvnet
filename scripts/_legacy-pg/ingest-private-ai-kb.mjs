#!/usr/bin/env node
/**
 * Ingest kb-private-ai-deployment.json into ask_ruvnet.architecture_docs
 *
 * Fields ingested:
...
const JSON_PATH = path.join(__dirname, '..', 'docs', 'kb-private-ai-deployment.json');
...
async function ingest() {
  console.log('Private AI Deployment KB Ingestion');
  console.log('===================================');
  ...
  try {
    for (let i = 0; i < entries.length; i++) {
      ...
      const docId = `private-ai-${hash(titleStr)}`;
      const fileHash = hash(contentStr);
      const filePath = `knowledge/private-ai-deployment/${docId}`;
