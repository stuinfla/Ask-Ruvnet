#!/usr/bin/env node
/**
 * Ingest kb-hardware-vendor-comparison.json into ask_ruvnet.architecture_docs
 *
 * Fields ingested:
...
const JSON_PATH = path.join(__dirname, '..', 'docs', 'kb-hardware-vendor-comparison.json');
...
async function ingest() {
  console.log('Hardware Vendor Comparison KB Ingestion');
  console.log('========================================');
  ...
  try {
    for (let i = 0; i < entries.length; i++) {
      ...
      const category = entry.category || 'vendor-comparison';
      ...
      const docId = `vendor-hardware-${hash(titleStr)}`;
      const fileHash = hash(contentStr);
      const filePath = `knowledge/vendor-hardware-comparison/${docId}`;
    ...
    try {
      const countResult = await countClient2.query(
        `SELECT COUNT(*) AS total FROM ask_ruvnet.architecture_docs
         WHERE doc_id LIKE 'vendor-hardware-%'`
      );
  ...
  console.log(`  DB rows (vendor-hardware-*): ${dbCount}`);
}
